...
describe("validateSubjectSelection", () => {
  it("requires at least one room for a per-room template", () => {
    expect(validateSubjectSelection({ kind: "ROOM", count: 0 })).toMatch(
      /per-room/,
    );
  });

  it("accepts one or many rooms for a per-room template", () => {
    expect(validateSubjectSelection({ kind: "ROOM", count: 1 })).toBeNull();
    expect(validateSubjectSelection({ kind: "ROOM", count: 12 })).toBeNull();
  });

  it("allows zero rooms when the template is not per-room", () => {
    expect(validateSubjectSelection({ kind: "NONE", count: 0 })).toBeNull();
  });

  it("caps the batch size", () => {
    expect(
      validateSubjectSelection({ kind: "ROOM", count: MAX_ROOMS_PER_CREATE }),
    ).toBeNull();
    expect(
      validateSubjectSelection({ kind: "ROOM", count: MAX_ROOMS_PER_CREATE + 1 }),
    ).toMatch(/at most/);
  });
});
...
describe("validateSubjectSelection — non-room kinds", () => {
  it("names people, not rooms, for a per-assignee template", () => {
    const msg = validateSubjectSelection({ kind: "ASSIGNEE", count: 0 });
    expect(msg).toContain("person");
    expect(msg).not.toContain("room");
  });

  it("names tasks for a per-task template", () => {
    const msg = validateSubjectSelection({ kind: "TASK", count: 0 });
    expect(msg).toContain("task");
  });

  it("rejects a selection on a property-wide checklist", () => {
    expect(validateSubjectSelection({ kind: "NONE", count: 2 })).not.toBeNull();
    expect(validateSubjectSelection({ kind: "NONE", count: 0 })).toBeNull();
  });

  it("caps every kind at MAX_ROOMS_PER_CREATE", () => {
    for (const kind of ["ROOM", "ASSIGNEE", "TASK"] as const) {
      expect(
        validateSubjectSelection({ kind, count: MAX_ROOMS_PER_CREATE }),
      ).toBeNull();
      expect(
        validateSubjectSelection({ kind, count: MAX_ROOMS_PER_CREATE + 1 }),
      ).not.toBeNull();
    }
  });
});
