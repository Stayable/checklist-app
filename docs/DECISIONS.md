...
4. **Zoho Desk dropped for network tickets** — ticketing is native (`Ticket`/`TicketNote` tables, in-app UI). Supersedes the ON-HOLD `docs/ITTicketingPlan_RISE8_072426.md` for the network-ticket use case specifically; that doc's Zoho evaluation was for a different (checklist-issue) ticketing surface and is otherwise unaffected.
...
- Zoho Desk is now confirmed out of scope for network tickets specifically; the checklist-issue ticketing question in `docs/ITTicketingPlan_RISE8_072426.md` remains separately ON-HOLD.
