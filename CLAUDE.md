...
**Next milestone:** **Blocked items** (need user/decision): (1) **R2** bucket + tokens + CORS → `/api/photos/presign` + `/api/photos/save`, wire `/photo-test`, filler photo upload, queue/review thumbnails, resolution-photo requirement; (2) **on-device** PWA install + Kate's `/ios-spike` GPS GO/NO-GO; (3) **Resend creds** → activation email + submit/flag emails (notification_log plumbing already in place); (4) **real template question content** (Karla/Christopher). **Unblocked next-up:** record the **alpha demo for Rob** (Phase 4 exit), then Phase 5 / Week 5 — recurring rules UI + 5 AM ET generation cron + bulk create + assignment policies + **invalidation flow incl. ADR-014 field-request→manager/admin-approval sub-flow** + unassigned-queue digest (Resend-gated). Also worth doing: Vitest on `lib/rbac.ts`. **Review checkpoint:** `pnpm dev` → log in as LL manager (`manager.lakeland@rentstayable.com` / `ChangeMe!2026`) → Home → Review queue (2 seeded SUBMITTED Arrivals: Rm 104 clean, Rm 105 has a FAIL) → open Rm 105 → Flag w/ priority → check `/issues` SLA target → resolve w/ note → `/admin/sla` as admin.
