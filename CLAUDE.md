...
- **✅ PER-PROPERTY ROUTING PROVEN on KW (2026-08-02).** Test ticket created + resolved, built with the real `buildTicketCreatedMessage` / `buildResolutionReply` and dispatched through the real `resolveTeamsWebhook` — reported `via TEAMS_WEBHOOK_URL_KW`, `rerouted=false`, confirming it used the property's own URL rather than falling back to General. The probe **refuses to send if `rerouted` is true**, so it can't accidentally "pass" by landing in General. ⚠ **7 property URLs remain individually unproven** (JN/JW/KE/LL/OR/SA/DP) — a mis-pasted URL still resolves, since routing only checks presence, and would surface only on that property's first real ticket.
