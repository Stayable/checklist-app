...
- 🧑 **Blocking handoff:** (a) apply migration `20260801120000` to prod **then** deploy (DB first — new code reads `escalatedAt`); (b) set the 9 `TEAMS_WEBHOOK_URL_*` in Vercel Production — prod has only the legacy var, which now serves General alone, so every property would reroute; (c) **warn Gerardo** the first sweep flushes all 14 open tickets at 5/tick (they are genuine, but it will look like 14 new faults); (d) let me send a live probe post.
