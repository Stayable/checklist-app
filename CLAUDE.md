...
**As of:** May 30, 2026
**Phase:** Week 1 in progress — Mon (i18n + datetime), Tue (Auth.js login), Wed (Prisma schema), **Thu (PWA shell)** + **Fri (photo POC client)** all shipped + committed. Only device-verification + R2-dependent steps remain in Week 1.
**Current week:** Week 1 (all writable foundation code committed; everything left is blocked on hardware or the user's R2 account — see below)
**Last milestone:** **Thu PWA shell + Fri photo POC client + auth-throttle tests shipped 2026-05-30 (commit `180b12d`)** — (a) PWA: `app.webmanifest` (scope `/`) + placeholder SVG icons (192/512/maskable), hand-rolled service worker (precache shell + network-first nav + `offline.html`) registered prod-only via `lib/pwa.ts` — **deliberate Workbox stand-in until `generateSW` works with Turbopack**; bilingual `InstallPrompt` (Chrome `beforeinstallprompt` + iOS A2HS guide), `/install` page, `OnlineStatus` header indicator; manifest + apple-web-app meta + themeColor in root layout. Left the `/ios-spike`-scoped `manifest.webmanifest` untouched. (b) Photo POC client: `lib/image.ts` shared compress (1920px/q85) + GPS helper factored from the spike, `/photo-test` page (camera, compress, separate GPS, EXIF read via `exifr`). **Upload (presign/save) intentionally NOT written — blocked on R2.** (c) `vitest` + `lib/auth-throttle.test.ts` (9 cases: 5-strike lock, 15-min window reset/boundary, 30-min unlock, success clear). **All green: typecheck, lint, 9/9 tests, prod build.** ES strings machine-drafted pending bilingual reviewer (open q#5). *Prior milestone: Tue Auth.js v5 login (`7d81b96`); Mon+Wed foundation (`384ca62`/`7879b91`/`4945fe6`); `/ios-spike` (`9c4792c`). Prisma pinned 6.x. Seed admin `admin@rentstayable.com` / `StayableCheck` (temp, rotate before prod).*
**Next milestone:** All remaining Week-1 items are **blocked on the user/hardware**, not code: (1) **R2 setup** — bucket `rise8-ops-staging` + scoped tokens + CORS (user's Cloudflare account), then write `/api/photos/presign` + `/api/photos/save` and wire the upload button into `/photo-test`; (2) **on-device verification** — install PWA on real iPhone + Android; Kate's `/ios-spike` standalone GPS run → **iOS GO/NO-GO**; (3) record results in `docs/PWA_TEST_RESULTS.md`. Unblocked next-up if continuing: **Phase 2 / Week 2** (full schema, admin user CRUD, RBAC middleware, property picker).
