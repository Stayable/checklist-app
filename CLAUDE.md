...
- **Live probe: General channel returned HTTP 202** (Kyle's call, General only). Payload deliberately included an em dash + `Cámara` to exercise the `charset=utf-8` pairing that otherwise 400s. ⚠ **202 = accepted, not rendered** — the flow 202s anything it takes, so a human still has to look at the channel. The other 8 URLs are the same mechanism but **individually unproven**; a mis-pasted one would only surface on that property's first ticket.
