...
**⏳ PLAN 2 (next) BLOCKED ON RESEND:** auth/OTP + user hard-delete + set admin pw `StayableAdmin`. Needs **`RESEND_API_KEY` + verified from-address** (still not in env). Plans 3–7: template builder+manual create; completed view+home revamp+photo metadata; manager dashboard+reports+PDF; recurrence polish; template hard-delete.
