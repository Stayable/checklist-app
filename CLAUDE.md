...
**As of:** June 23, 2026 (evening)
**Phase:** **Phase 4 (ALPHA) + Plan 1 desktop shell + Phase 5 recurrence ALL LIVE IN PRODUCTION** at **https://ops.rentstayable.com**.
**🆕 2026-06-23 PM — Plan 1 (AppShell) + Phase 5 MERGED TO PROD.** Kate logged into prod, saw the old narrow mobile-column layout + inconsistent admin header, asked for a desktop-first responsive dashboard. That's exactly Plan 1 — already built but unmerged. She **skipped the Preview review and chose merge-to-prod.** FF merge `b55e073..9cf069d`; deploy `dpl_HKymj4…` READY; `ops.rentstayable.com` now serves the **navy left-sidebar desktop shell** (collapses to bottom-bar on mobile), admin folded into the same shell. Kate confirmed "much better." The same merge **also carried Phase 5 recurrence (engine + cron + `/rules`) to prod** — see cron note. Two fixes rode along in merge commit `9cf069d`: (a) PWA install prompt now **field-staff + mobile only** (new `isFieldStaff()` in `lib/rbac.ts`; desktop/managers never see it); (b) cron route **fail-closed in prod**.
**(prior) Phase 4 live since 2026-06-19:** review + issues + R2/photo pipeline ADR-015 + issue-resolution photos ADR-016 + Connecteam-familiar UI structural pass ADR-017.
**Current week:** Week 5 (Recurrence). Phase 4 remaining: alpha demo for Rob; Resend emails.
...
**🟢 PLAN 1 DONE + IN PROD (2026-06-23):** Unified AppShell + property scope. `lib/nav.ts` + `lib/property-scope.ts` (tested); `components/shell/{AppShell,ShellChrome,PageHeader}.tsx` (desktop navy sidebar `lg+`, mobile bottom-bar + top bar `<lg`, content `max-w-6xl`); mounted in root layout; Home/admin/review/issues/rules migrated; review/issues narrowed by active property; `AppNav.tsx`+`BottomNav.tsx` deleted. **MERGED to main + deployed to prod** (was the merge gate; Kate skipped Preview). 73/73 tests, build 19 routes.
**⏳ PLAN 2 (next) BLOCKED ON RESEND:** auth/OTP + user hard-delete + set admin pw `StayableAdmin`. Needs **`RESEND_API_KEY` + verified from-address** (still not in env). Plans 3–7: template builder+manual create; completed view+home revamp+photo metadata; manager dashboard+reports+PDF; recurrence polish; template hard-delete. **Plan 3 (template builder) is fully unblocked — next-up if not doing Plan 2.**
**🔀 Branch state (2026-06-23):** `main` is at `9cf069d` (everything merged). Feature branch `claude/rise8-operations-platform-rv9B6` was FF-merged into `main` and is even with it. Start next work fresh off `main` or keep using the branch.
