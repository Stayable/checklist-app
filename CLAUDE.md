...
**As of:** June 27, 2026
**Phase:** **Phase 4 (ALPHA) + Plan 1 desktop shell + Phase 5 recurrence LIVE IN PRODUCTION** at **https://ops.rentstayable.com**. **Plans 2–5 (auth/OTP, template builder, completed view, dashboard/reports/PDF) being merged to prod 2026-06-27** (full-branch merge; ⚠ requires `RESEND_API_KEY` + `RESEND_FROM_EMAIL` in Vercel Production or login locks out — untrusted-device OTP send still to be validated live).
...
**🔀 Branch state (2026-06-27):** branch `claude/rise8-operations-platform-rv9B6` carries Plans 2–5 (auth/OTP, template builder, completed view, dashboard/reports/PDF). `main` had one extra docs-only commit (`2a55336`); merged back into the branch 2026-06-27 (only CLAUDE.md/TODO.md conflicts, resolved). **Plan 1 (AppShell) + Phase 5 recurrence already in prod since 2026-06-23** (FF merge `b55e073..9cf069d`; desktop navy left-sidebar shell, admin folded in; PWA install prompt field-staff+mobile only; cron route fail-closed in prod). **⚠ ENABLING-GATE for this merge: `RESEND_API_KEY` + `RESEND_FROM_EMAIL` must be in Vercel Production with a verified sending domain BEFORE the prod deploy goes live — every prod device is untrusted on first login, so OTP send must work or login locks out. Validate a real OTP send first; rollback candidate is the current prod deployment.**
