...
- **NEXT BUILD: T11 UniFi poller** — `lib/network/unifi-api.ts` + `unifi-poll.server.ts`, diffing device state into the existing `ingestWebhook` pipeline (tickets/timers/mass-outage/UI unchanged). N2/N3/N4 are **build requirements, not follow-ups.**
