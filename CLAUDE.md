...
**🔁 CONTRACTOR SCHEDULE + DAILY DASHBOARD IS BACK IN SCOPE — BUILT HERE, FROM SCRATCH (Kyle, 2026-08-11).**
...
- **🚩 OPEN AND LOAD-BEARING — the integration seam.** "In conjunction with the contractor update project" is not yet pinned down: does **this** repo own contractor and job records, or mirror/read them from Kyle's separate system? The answer changes the schema. Default assumption unless Kyle says otherwise: **this repo owns its own minimal records**, designed so a later import/sync can populate them — the only option buildable without that project's API.
...
- **BUILD STARTED SAME DAY — 4 of 9 tasks done, all reviewed clean, branch-local.** Branch `feat/contractor-scheduling-v2` off `main` @ `610119b`, HEAD **`06a33bb`**. Spec `docs/superpowers/specs/2026-08-11-contractor-scheduling-design.md`, plan `docs/superpowers/plans/2026-08-11-contractor-scheduling.md`, live ledger `.superpowers/sdd/2026-08-11-contractor-scheduling/progress.md` (**read the ledger to resume — it is authoritative over this block**).
  ...
  - **Tests 519 → 623/623**, zero regressions at every step; typecheck, lint, build clean throughout.
- **🛑 D-5 (job lifecycle) WAS STOPPED MID-WORK** (battery). `app/maintenance/jobs/` sits **untracked, uncommitted, ungated, unreviewed**. **Delete it and re-dispatch — do not salvage.** A plausible-looking actions file that skipped terminal-immutability or the server-side eligibility re-check is worse than no file.
- **🚩 A PLAN DEFECT OF MINE:** `Contractor.company` exists at `prisma/schema.prisma:840` and is displayed, but is absent from `contractorSchema`, so nothing can ever write it — Task 1's field list had it, Task 3's schema list didn't. Adjudicated: **add the optional field + form input, do not remove the display.** Queued before the nav task.
- **⚠ Nothing applied to any database and nothing deployed.** Migration `20260811120000_add_contractor_scheduling` is authored only. Additive ⇒ prod order is **DB first, then code**. Directory ships empty; no seed roster by choice (the last one leaked placeholder phone numbers into the baseline).
