...
**As of:** June 15, 2026
**Phase:** Weeks 1–3 **LIVE IN PRODUCTION** at **https://ops.rentstayable.com** (Nunito font + Stayable navy palette). **Phase 4 + R2/photo pipeline + issue-resolution photos (ADR-016) + Connecteam-familiar UI structural pass (ADR-017)** all on branch `claude/rise8-operations-platform-rv9B6` — NOT yet on `main`/prod. Remaining in Phase 4: alpha demo for Rob; Resend email rows.
**Current week:** Week 4→5 boundary (ALPHA). Review + issues + photos end-to-end except emails (Resend).
**🔀 Branch state (2026-06-15):** branch and `main` had diverged (`main` = prod-deploy + branding/fonts; this branch = Phase 4 + photos). **Merged `origin/main` into this branch 2026-06-15** so it now has both. **Next: re-skin the redesign to brand tokens** (`#041E42` navy / `#0091F5` blue; Nunito already global) — the structural pass used placeholder `slate-900`.
...
- **R2 envs:** the 4 `R2_*` vars are **now in Vercel Production** (confirmed 2026-06-20 via `vercel env ls production`, added ~5h prior) — merge gate to `main` is CLEARED. Still **not** in Preview, so authed-route Preview testing of `/review`/`/issues`/`/` would throw on presign unless added there too.
