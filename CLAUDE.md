...
**As of:** May 30, 2026
**Phase:** Weeks 1–3 shipped & **LIVE IN PRODUCTION** at **https://ops.rentstayable.com** (merged to `main`, commit `1555776`). Login fixed, Stayable branding applied. Remaining blockers: R2 (photo upload), Resend (activation email), real question content, self-invalidation decision, Adobe font auth.
...
- **GitHub→Vercel auto-deploy is FLAKY:** the merge push deployed, but a later `git push origin main` produced no deployment. **Reliable path: `vercel --prod --yes` (CLI, now installed + authed as `admin-15537530`).** Vercel git integration needs checking.
