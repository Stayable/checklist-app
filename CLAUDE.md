...
**As of:** June 25, 2026
**Phase:** **Phase 4 (ALPHA) LIVE IN PRODUCTION** at **https://ops.rentstayable.com** (merged + deployed 2026-06-19: review + issues + R2/photo pipeline ADR-015 + issue-resolution photos ADR-016 + Connecteam-familiar UI structural pass ADR-017). **Phase 5 recurrence backend + rules UI built 2026-06-20 — on branch, NOT yet on `main`/prod.**
...
**⏳ PLAN 2 BLOCKED ON RESEND:** auth/OTP + user hard-delete + set admin pw `StayableAdmin`. Needs **`RESEND_API_KEY` + verified from-address** (still not in env). User (Kyle/Kate) handling Plans 1 & 2 (credential/merge-gate work) in parallel.
...
**⏭ PLANS 5–7 (next):** 5 = manager dashboard (`/dashboard` incomplete+with-issues alerts) + reports (daily completeness + issues-found) + PDF export; 6 = recurrence polish (after Kate reviews `/rules`); 7 = template hard-delete (LAST, row-count-confirmed). **Plan 5 PDF will reuse `PhotoFigure`'s time+geo display.**
**🔀 Branch state (2026-06-20):** branch `claude/rise8-operations-platform-rv9B6` is **ahead of `main`** by the Phase 5 commits (`db6a03c` engine, `d3bec23` cron, `8ee7b07` rules UI) + doc reconciliation. **Pushed to origin.** Merge to `main` held for go-ahead — see two prod-enablement gates below.
...
**⛔ Phase 5 prod-enablement gates (before/at merge to main):** (1) **set `CRON_SECRET` in Vercel Production** — the cron route auth is open when the secret is unset; (2) merge branch → `main` (the cron route + `/rules` only reach prod on merge). Cron runs `0 9 * * *` UTC = 5 AM EDT / 4 AM ET in winter.
