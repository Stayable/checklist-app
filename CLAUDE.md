...
**As of:** June 2, 2026
**Phase:** Weeks 1–3 shipped & **LIVE IN PRODUCTION** at **https://ops.rentstayable.com**. Login fixed, Stayable branding applied, **fonts now rendering correctly (Nunito)**. Remaining blockers: R2 (photo upload), Resend (activation email), real question content, self-invalidation decision, optional Adobe font auth.
**Current week:** Week 3 (Phase 3) complete; app deployed to prod. Next code phase is Phase 4 (manager review + issues).
**⚠️ Working on `main`:** this session committed straight to `main` (auto-deploys to prod). Feature branch `claude/rise8-operations-platform-rv9B6` is **behind `main`** — branch off `main` for new work.
**🔴 PROD/OPS NOTES (read before deploying again):**
- **DB:** Production uses the **same Neon DB** as local dev (`ep-falling-moon-apwovbb3`) — it holds the seeded test users + PLACEHOLDER question content. Not real data; fine for review, NOT go-live.
- **Login outage fixed 2026-05-30:** Vercel Prod `DATABASE_URL`/`DIRECT_URL` were **empty strings** → Prisma threw → NextAuth `Configuration` error. Set both via `vercel env add ... production --force` to the Neon connection from `.env.local`; redeployed. Login verified (admin `302 → /`, ADMIN session). Env vars are **sensitive** (can't be read back via `vercel env pull` — shows `""`; that's normal, not empty).
- **GitHub→Vercel auto-deploy:** worked cleanly on 2026-06-02 — both `git push origin main` triggered prod builds with no CLI nudge. Earlier flakiness not reproduced; CLI `vercel --prod --yes` remains the fallback if a push ever fails to deploy.
...
**Last milestone:** **Fixed fonts never rendering + switched to Nunito 2026-06-02 (commits `e12ce23` + `605105b`)** — production text was rendering in the browser **default serif**, not any brand font. Root cause: next/font sets the CSS variable (`--font-roboto`/`--font-nunito`) on `<body>` via its className, but `globals.css` applies `font-family` on `<html>` (`html { @apply font-sans }` → `font-family: var(--font-sans)` → `var(--font-nunito)`); on `<html>` the variable was undefined → invalid → serif fallback for the whole document. This was broken for Roboto too — no font swap had ever actually rendered. **Fix:** moved the font-variable classes to `<html>` and `font-sans` to `<body>`. Also swapped Roboto+Quicksand → **Nunito** (free rounded-geometric stand-in for Adobe Urbane Rounded; one family, body + headings, weight carries hierarchy; Geist Mono kept). **Verified** at CSS/HTML level on a local prod server AND on live `ops.rentstayable.com`: `<html>` carries the Nunito var class and `html{font-family:var(--font-nunito)}` resolves. Lesson: checking that font files are *referenced* ≠ confirming they're *applied* — verify computed font, not just `<link>`s. *Prior milestone below.*
**Earlier milestone:** **Deployed to production + login fix + Stayable branding 2026-05-30 (commit `1555776`)** — (a) merged Weeks 1–3 to `main` → live at ops.rentstayable.com (custom domain + DNS already attached); (b) **fixed the login outage** (empty prod DB env, see PROD NOTES); (c) **applied Stayable brand foundation** from rentstayable.com: Roboto (body) + **Quicksand** (headings, stand-in for Adobe **Urbane Rounded** which is domain-locked), palette tokens navy `#041E42` / blue `#0091F5` / sky `#91D1FA` / gold `#FDDA24` in `globals.css @theme`, primary actions recolored slate→navy on login/filler/admin/install. Verified live: fonts + navy in served CSS, login works. Full screen-by-screen reskin remains Phase 7. *Prior milestone below.*
**Older milestone:** **Phase-3 checklist filling shipped 2026-05-30 (commit `a6ea416`)** — pure logic core `lib/checklist-logic.ts` (conditional `show_if` visibility + per-type validation) with **14 unit tests**; ET-anchored "Today" home (`etDateOnly`); `/checklists/[id]` filler rendering all 11 question types with live conditional visibility, client+server validation, `SignaturePad` (canvas→PNG dataURL), native-camera photo capture + client compress (R2 **upload deferred** → `{count, pendingUpload}`), IndexedDB draft autosave (`idb`), submit action (persist responses → SUBMITTED + audit → confirmation); first-login `LocalePrompt` (HK/PA/MT → `users.locale`); EN+ES strings for all field surfaces (ES machine-drafted, pending reviewer); seed adds 3 ASSIGNED Arrival instances for the LL HK so the flow is demoable pre-cron. **All green: typecheck, lint, 23/23 tests, prod build (11 routes).** *Runtime click-through not yet exercised — login interactive.* *Prior: Phase-2 schema + admin console + RBAC (`b366895`, `7777710`); Week-1 PWA + photo POC + auth tests (`180b12d`).*
...
- **Deploy to production early** (2026-05-30) — Weeks 1–3 went live at ops.rentstayable.com mid-build (per Kate "deploy immediately"). Prod shares the dev Neon DB + seeded test data for now; not a real go-live (cutover still Week 14).
- **Brand foundation = rentstayable.com** (2026-05-30) — Roboto body + **Quicksand headings as a stand-in** for Adobe Urbane Rounded (domain-locked); palette navy/blue/sky/gold. Apply foundation now; full reskin stays Phase 7. Swap to real Urbane Rounded once Adobe kit `dsq0zcq` is authorized for the ops domain.
