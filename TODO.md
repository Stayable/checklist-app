...
| 11 | ✅ **General channel PROVEN — Kyle confirmed the post rendered.** Em dash + `Cámara` both survived, so the `charset=utf-8` pairing works live | ⚠ The other 8 URLs are the same mechanism but **individually unproven** — a mis-pasted one would only surface on that property's first ticket |
| 12 | 👀 **Third probe sent (structured card digest) — eyeball whether the table lines up now** | The 2nd probe proved a padded text table **collapses** in a card TextBlock. Rebuilt as a real ColumnSet (`753c2d9`) and re-probed. If it still doesn't align, the fallback is one line per property — no alignment dependency at all |
