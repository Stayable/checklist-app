...
| 11 | ✅ **General + KW channels PROVEN.** General confirmed rendering by Kyle (em dash + `Cámara` survived, so `charset=utf-8` works live). **KW verified 2026-08-02** with a test ticket created + resolved through the real `buildTicketCreatedMessage`/`buildResolutionReply` builders and the real routing — resolved `via TEAMS_WEBHOOK_URL_KW`, `rerouted=false`, so it used the property's own URL and not a General fallback | ⚠ **7 property URLs still individually unproven**: JN · JW · KE · LL · OR · SA · DP. A mis-pasted one resolves fine (the routing only checks presence) and would surface only on that property's first real ticket. Offered to Kyle; not sent, since each one reaches a different manager's channel |
