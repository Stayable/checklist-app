...
| D-14f | P1 | [ ] | Prove on a **preview** deploy with a replayed payload before the pipeline points at production (contract §10 steps 1–3). ⚠ **Nothing below the line has run:** the migration is applied to no database, nothing is deployed, and no request has ever hit this route. 705 tests, clean typecheck + lint, and a build that registers `/api/webhooks/contractor-update` are the whole of the evidence. **The pipeline half does not exist either**, so there is nothing to receive from yet |
| D-14i | P1 | [ ] | 🧑 Set `CONTRACTOR_UPDATE_SECRET` in Vercel (**and give the pipeline the same value** for its Key Vault `checklist-app-webhook-secret`). Until it is set, production **rejects every request** — which is the intended fail-closed default, not a bug |
