...
| D7 | P2 | [ ] | **L2 — Automated WhatsApp** (auto-send + auto-reply + escalation ladder). **Spec written 2026-07-28: `docs/component-ii/WhatsAppAutomationSpec_RISE8_072826.md` (§SPEC-7).** Delivery architecture already exists (the Teams queue+cron pattern); what's missing is a Meta account, 4 UTILITY templates in EN+ES, and a client. **Vendor decided 2026-07-29: Twilio as BSP** — solves the dedicated-number problem, and its **Sandbox lets the whole pipeline be built and tested before Meta verification clears** (going live then = an env-var swap). Twilio does NOT remove Meta verification, template approval, the 24h window or opt-in. 🧑 Kyle: Twilio account + number + sandbox (minutes) unblocks the build. Note: staff notifications deliberately stay in-app/push — WhatsApp is for contractors only | §Q17 |
