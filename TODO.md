...
## 🎯 START HERE (updated 2026-07-30, later session)
...
|---|---|---|
| 1 | 🧑 **Add the 3 fabric keys to Vercel Production** — `UNIFI_API_KEY_CENTRAL` · `UNIFI_API_KEY_INDEPENDENT` · `UNIFI_API_KEY_NORTH`, then deploy | ✅ **§Q1 CLOSED 2026-07-31 — full-estate visibility proven locally.** All three keys verified: 20 host entries, 0 failures, **16 consoles across all 8 properties, 640 devices, 0 monitoring-blind**. Registry updated. **Vercel still holds the retired `UNIFI_API_KEY` / `UNIFI_API_KEY_2` names, which this code no longer reads — set the three new vars BEFORE deploying or polling stops dead.** ⚠ First tick will emit **72 PROBLEM events** for devices offline right now (46 of them Orlando cameras) — see §Q28 before deploying |
...
| 7 | 🧑 **Orlando: 46 of 92 cameras offline right now** | A real operational finding the monitoring surfaced. Independent of any build work |
