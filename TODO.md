...
`outputs/RemotePMScheduling_RISE8_090726.md`, **which A6 below cites as the recurring-rules reference
sheet, so that pointer had been dangling for anyone who cloned the repo.** Typecheck clean, tree clean,
**not pushed**. The ordered list below is unchanged and still correct.
...
**⏳ THE CAPTURE IS NOW A DAY OLD — re-read the sheet before loading.** Taken 2026-08-25 ~11:20 ET; it is 2026-08-26 as of this checkpoint. The sheet takes daily WhatsApp updates and status changes, so the `In Progress` rows have almost certainly moved and Tuesday's narratives are missing. Loading it into an empty calendar still creates every job correctly — the **statuses would just be a day behind**, and the sheet wins on the next run anyway. Re-capture is the clean option and costs one read.

**📸 The 08/24–08/28 week is captured** — `scripts/data/smartsheet-contractor-schedule-snapshot.json`
