...
| Task | Pri | Status | What |
|---|---|---|---|
| D-14a | **P0** | [!] | ⚠ **Backfill 13 contractor phone numbers (E.164) — blocks the whole feature.** The contract resolves a job on `(workDate, contractorPhone)`, phone first. **Verified 2026-08-13 against prod: 0 of 13 contractors have a phone or WhatsApp** (`scripts/check-contractor-identity.ts`, read-only) — the import deliberately omitted them ("will add later"). So every update would fall to exact-string name matching across two rosters nobody has diffed, and the failure is **silent**: unresolved updates land in `ContractorDailyNote` and the calendar simply looks unchanged. §Q40 |
| D-14b | P1 | [ ] | One additive migration: `DELAYED` on `ContractorJobStatus` (closes §Q35) + a `ContractorUpdate` table with `messageSid @unique`. Separate from `RawWebhookPayload` — its `source` column is the **device** enum `DeviceSource`, and a unique key on the capture row would 500 on Twilio's retry instead of returning the contract's `duplicate: true` |
| D-14c | P1 | [ ] | `lib/contractor-update.ts` — pure: Zod payload, version gate, E.164 normalization, status map, and a decision function returning `duplicate` / `unsupported-version` / `apply-status` / `note-only` / `daily-note` / `ambiguous`. All of §7–§8 becomes table-driven tests with no DB and no HTTP |
| D-14d | P1 | [ ] | ⚠ **`STATUS_MAP` in `sync-contractor-schedule-from-smartsheet.ts` must change in the same commit as the enum.** Left at `Delayed → PLANNED` it does not merely revert a delayed job — it appends a false `SYSTEM` note ("Status changed to PLANNED — Smartsheet now says \"Delayed\"") to an **append-only** thread that cannot be corrected. Ship with a test asserting every enum value appears in `JOB_STATUS_ORDER` and in exactly one of `OPEN_JOB_STATUSES`/`TERMINAL_JOB_STATUSES` — the `in:` arrays exclude a new value silently, `notIn:` includes it silently |
| D-14e | P1 | [ ] | `POST /api/webhooks/contractor-update` — capture, HMAC via the existing `lib/network/hmac.ts`, one transaction. Fail-closed on an unset secret in production, as the UniFi receiver does |
| D-14f | P1 | [ ] | Prove on a **preview** deploy with a replayed payload before the pipeline points at production (contract §10 steps 1–3) |
