...
| 10 | 🧑 **Fix the Gerardo @-mention (Kyle: Monday)** | The **email works and is live now** — any NEW escalation from here emails `NETWORK_ESCALATION_EMAIL` (default gerardo@rentstayable.com) and posts to General. What's missing is only the real Teams `@`-tag, which needs the Power Automate flow to build a mention entity. Until then the post names him in plain text |
